...
func createHandler(w http.ResponseWriter, r *http.Request) {
	...
	"<script> function myFunction(){ var resulta = document.getElementById('resulta'); var elementUrl = document.getElementById('url'); var elementDuration = document.getElementById('duration'); var elementPwd = document.getElementById('pwd'); resulta.innerHTML = ''; resulta.setAttribute('href', '');   $.post('/api/create',{url:elementUrl.value,duration:elementDuration.value,password:elementPwd.value}, function(data,status){if(status = 'success') { var shortUrl = 'http://' + data.domain; if(data.port != '80') { shortUrl = shortUrl + ':' + data.port; } shortUrl = shortUrl + '/' + data.key; if(elementPwd.value !='') {shortUrl = shortUrl + '?pwd=' + elementPwd.value}  resulta.innerHTML = shortUrl;resulta.setAttribute('href', shortUrl); if(elementDuration.value != ''){alert('请在 ' + elementDuration.value + 's 内访问')}} else {alert('错误：' + data);}}); }</script>" +
	"</body> </html> ")
}
